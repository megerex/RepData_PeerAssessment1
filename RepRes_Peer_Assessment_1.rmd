...
+ Identify R Working folder.    
    ...
    The current R Markdown working folder is: **[`r getwd()`]**.
...
```
+ Result    
  The daily step mean is `r DailySteps.mean` and the daily step median is `r DailySteps.median`.
...
+ Result    
It is thus revealed that the `r rownames(my.data.maxIntervalSteps[1,])`th interval at `r my.data.maxIntervalSteps$interval` minute of the day has the highest average step of `r my.data.maxIntervalSteps$steps` steps.
...
+ Result    
The dataset is thus with `r sum(my.data.NA)` missing values.
